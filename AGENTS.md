...
- Use theme-aware color functions from `app_theme.dart`:
  ...
  - `accentColor(context)` - the accent as **ink**: text, icons, hairlines
  - `accentFillColor(context)` - the accent as a **ground**: anything with a
    label or glyph painted on top of it
  - `accentMutedColor(context)` / `accentDimColor(context)` - the two opaque
    washes between the background and the fill, for ramps like the heatmap
  - `onAccentColor(context)` - text/icons drawn *on* `accentFillColor`
...
- **`accent` is ink, `accentFill` is a ground - never swap them.** They are
  ...
  light mode. Painting `onAccentColor` on `accentColor` scores 3.63:1 on every
  accent, which is how 23 filled buttons and snackbars ended up failing AA
- Anything drawn on a coloured ground asks for its foreground:
  - accent ground (filled button, selected chip/tab, accent snackbar) →
    `accentFillColor(context)` for the ground, `onAccentColor(context)` on it
...
- Use `accent.withAlpha(value)` for splash/highlight effects. A translucent
  wash is fine as a *tint*; it is not fine as a step in a ramp that carries
  meaning, because it composites differently on a card than on the page - use
  the opaque `accentMuted`/`accentDim`/`accentFill` steps there
- `test/theme_contrast_test.dart` enforces all of the above for every accent in
  both modes, including a source scan for the ink/ground swap. If it fails, the
  colour is wrong - don't relax the target
