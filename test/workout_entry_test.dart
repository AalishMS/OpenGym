...
void main() {
  ...
  testWidgets('Week 1 prescription autosaves without creating a PR', (
    tester,
  ) async {
    tester.view.devicePixelRatio = 1;
    tester.view.physicalSize = const Size(400, 880);
    ...
    final plan = WorkoutPlan(
      id: 'week-1-plan',
      splitId: 'week-1-split',
      name: 'Week 1 Strength',
      exercises: [
        ExerciseTemplate(
          name: 'Squat',
          sets: 1,
          setTargets: [SetTemplate(reps: 5, weight: 120)],
        ),
      ],
    );
    await tester.runAsync(
      () => Hive.box<WorkoutPlan>(HiveService.plansBox).put(plan.id, plan),
    );
    ...
    await tester.pumpAndSettle();
    final draft =
        HiveService.getSessionForPlanAndWeek(plan.name, 1, plan.splitId)!;
    expect(draft.exercises.single.sets.single.completed, isFalse);
    expect(HiveService.getExercisePR('Squat', plan.splitId), 0);
  });
}
