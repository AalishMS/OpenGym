...
- Refined the History workout-details readout:
  ...
  - Moved saved PR marker notes (`New PR!`, `PR attempt`, and `PR`) beside their
    weight as a compact `PR` marker while preserving ordinary set notes below
    their set rows
...
- Refined Statistics around two focused charts:
  - Weekly training now leads with exercise-filtered volume load, independent
    ...
    fixed-width bars across all history, open at the latest week, and include
    inactive weeks, a pinned value axis, exact selection details, and a Latest action
  - Exercise progress keeps its own exercise, metric, and period controls, with
    Latest/Change/Best in period summaries, readable axes, and persistent workout
    selection details. Controls and charts share theme-aware surfaces
  - Removed Recent sessions and the weekly metric/period controls; records now
    follow exercise progress with quieter copy and more readable training values
  - Added aggregation, ISO boundary, independent-selector, scroll-state, and
    responsive chart tests; optional real-font previews are generated by
    `test/statistics_redesign_test.dart` with `OPENGYM_PREVIEW_DIR` and
    `OPENGYM_PREVIEW_FONTS` (a directory containing Manrope.ttf and JetBrainsMono.ttf)
